...
Sense owns its internal language and its Project -> Bridge language: what Sense emits and what it means. Lab owns later Bridge -> Interface presentation language for humans and should use product-agnostic Lab defaults, but must preserve Sense meaning unless Human override resolves a conflict. Shared spelling does not imply shared meaning.
