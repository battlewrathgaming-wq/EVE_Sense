...
| Runtime live IO authority | Operator toggle and main-process live IO policy. | Runtime control applies IO authority to local gamelog ingest, Passive and Threat provider gates, and Clipboard Acquisition gate paths. Local gamelog ingest is blocked at start/read/admission while I/O is off. | `runtime.live-io.snapshot` and `runtime.live-io.set-enabled` service commands. | Top `IO` button and diagnostics live IO field. | Backend-owned user authority over ingest behavior. | Enabled/disabled, blocked, last-observed/resting state, provider failure, no observation, and lane impact must remain explainable and distinct. | Provider failure, no scan, no observation, unavailable status, tactical safety, or broad historical storage. |
...
- I/O off means Sense is not allowed to ingest. Local gamelog start/read/admission, Clipboard Acquisition reads, and provider calls are gated; last-observed/resting display may remain visible without fresh ingest.
