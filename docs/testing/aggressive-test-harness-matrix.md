...
| Watcher containment and append-only behavior | `npm.cmd run verify:gamelog-watcher` | `npm.cmd run verify:gamelog-watcher-chaos` | Configured folders must resolve to the expected EVE `logs/Gamelogs` structure; active file reads must stay inside the watched folder; old files, replacements, and rejected lines must not replay or leak raw private lines in diagnostics. |
...
| Provider failure visibility | `npm.cmd run verify:provider-faults` | `npm.cmd run verify:http`, `npm.cmd run verify:passive-telemetry`, `npm.cmd run verify:threat-intel` | Passive Telemetry and Threat Intel provider faults stay lane-specific and distinct from live IO blocked; timeout, malformed, 429/500, stale cache, and ETag failures remain observable without live network. |
...
| `npm.cmd run verify:gamelog-watcher-chaos` | Yes | Append-only watcher chaos, active-folder containment skips, separator-like fs-watch filename rejection, symlink-file skip where feasible, same-size/larger replacement identity seeding, polling fallback, truncation/replacement handling, listener/parser failure isolation, hash-only rejection evidence. |
...
| `npm.cmd run verify:provider-faults` | Yes | Fixture-only Passive Telemetry and Threat Intel provider fault injection for zKill, ESI, malformed provider data, stale ESI cache/ETag revalidation failure, HTTP timeout/429/500/invalid JSON, and live IO blocked call prevention. |
