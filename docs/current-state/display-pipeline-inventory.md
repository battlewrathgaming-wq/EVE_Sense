...
| Manual Threat Intel target | Operator-entered text and target kind. | Threat service builds a scan request with target text, target kind, input source, lookback, and sample limit. | `threat.intel.scan` preload call and resulting Threat snapshot. | Threat acquisition/search surface and target display. | Deliberate operator-selected target inspection. | Target kind, resolver state, sample basis, and no-scan/failure/blocked states. | Automatic target finding, broad search, complete intelligence, or hidden provider action. |
| Clipboard-acquired target | Clipboard content read only during the armed/listening authority window. | Clipboard Acquisition ignores unchanged content, captures valid target text, seals, enters cooldown, and hands target to Threat Intel scan path. | `clipboard.acquisition.snapshot`, clipboard commands, and Threat scan request. | Clipboard widget, key chips, acquisition messages, populated Threat target. | Short visible authority window feeding the same Threat Intel boundary. | Listening/pulling/cooldown/blocked state and shortcut status must stay visible enough for trust. | Background clipboard monitoring or persistent listener mode. |
...
- Clipboard Acquisition must stay a short visible authority window.
...
| `sense.clipboard-window` | Preserve short visible clipboard authority while improving boundedness and shortcut feedback. | pressure-test | Clipboard Acquisition, Pulling, Listening, Cooldown | Background clipboard monitoring, persistent listener semantics, shortcut behavior changes. |
