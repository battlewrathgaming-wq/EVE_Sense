...
| `docs/features/clipboard-acquisition.md` | Feature contract | Defines I/O-gated `Control+\` permission action, focused/windowed authority window, duplicate suppression, and seal/cooldown behavior. | Safety/trust-critical. |
...
| `src/threat/` | Threat Intel and Clipboard Acquisition | Emits scan snapshots, target resolution, provider sample/cap states, clipboard lifecycle, and duplicate-suppression behavior. | Main Threat Intel bridge meaning source. |
...
| Clipboard Acquisition lifecycle and shortcuts | The I/O-gated `Control+\` permission action, no-read-when-off behavior, short authority window, duplicate suppression, and seal/cooldown lifecycle are central to current UX. | Sense Overseer / Human |
