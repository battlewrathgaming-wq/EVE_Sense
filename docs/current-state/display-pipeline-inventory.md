...
| Runtime live IO authority | Operator toggle and main-process live IO policy. | Current runtime control applies live IO enabled/disabled policy to Passive and Threat gates; ADR-0008 target behavior broadens this into no-ingest authority for local logs, clipboard, providers, and future ingest sources. | `runtime.live-io.snapshot` and `runtime.live-io.set-enabled` service commands. | Top `IO` button and diagnostics live IO field. | Backend-owned user authority over ingest behavior. Current implementation still needs ADR-0008 reconciliation for local gamelog ingest. | Enabled/disabled, blocked, last-observed/resting state, provider failure, no observation, and lane impact must remain explainable and distinct. | Provider failure, no scan, no observation, unavailable status, tactical safety, or proof that implementation is already fully ADR-0008 aligned. |
...
- I/O off must mean Sense is not allowed to ingest; current local gamelog ingest behavior still needs ADR-0008 reconciliation before this can be claimed implemented.
